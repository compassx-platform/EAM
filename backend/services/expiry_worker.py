...
def check_and_expire_permits(db: Session) -> List[Dict[str, Any]]:
    """
    Finds all active permits with custom_fields.expiry_date < now() and transitions them to EXPIRED.
    Uses ordinary propose_transition with actor_type='system' (Section 7.4).
    """
    now = datetime.now(timezone.utc)
    expired_results = []

    active_permits = db.query(Permit).filter(Permit.status == "Active").all()
    ...
    for p in active_permits:
        ...
        try:
            ...
            if exp_date < now:
                # Fire transition through standard propose_transition
                res = propose_transition(
                    ...
                    actor_type="system",
                    payload={"reason": f"Permit expired automatically at {now.isoformat()} (expiry was {expiry_str})"}
                )
            ...
            logger.error(f"Error checking expiry for permit '{p.id}': {str(ex)}")
